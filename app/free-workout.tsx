...
/** Discover-only logs/sessions for this catalog workout (not plan “Day X” sessions). */
function filterDiscoverRecords(rows: DayRecordRow[], scheduleWorkout: string | null): DayRecordRow[] {
  const w = scheduleWorkout?.trim() || null;
  if (!w) return [];
  const wn = normalizeWorkoutName(w);
  return rows.filter((r) => normalizeWorkoutName(r.title) === wn);
}

function mapDiscoverWorkoutLogDoc(d: QueryDocumentSnapshot, workoutTitle: string): DayRecordRow | null {
  const data = d.data() as Record<string, unknown>;
  if ((data as any)?.origin !== "discover") return null;
  const t = typeof (data as any)?.title === "string" ? (data as any).title : "";
  if (normalizeWorkoutName(t) !== normalizeWorkoutName(workoutTitle)) return null;
  ...
  return { id: `log-${d.id}`, title, planDay: null, startedAt, endedAt, elapsedSeconds, burnedKcal, met };
}

function mapDiscoverSessionDoc(d: QueryDocumentSnapshot, workoutTitle: string): DayRecordRow | null {
  const data = d.data() as Record<string, unknown>;
  if ((data as any)?.origin !== "discover") return null;
  const w = typeof (data as any)?.workout === "string" ? (data as any).workout : "";
  if (normalizeWorkoutName(w) !== normalizeWorkoutName(workoutTitle)) return null;
...
/**
 * Same timer/instruction/record flow as plan day-workout, but logs are tagged `origin: "discover"`
 * so they do not mix with plan-day records. Still writes dailyStats + workoutLogs for Home/Progress.
 */
function FreeWorkoutBody({ workoutType, workoutName }: { workoutType: WorkoutType; workoutName: string }) {
  ...
  const row = useMemo(
    () => ({ day: 0, type: workoutType, workout: workoutName }),
    [workoutType, workoutName]
  );

  const workoutDetail = useMemo(
    () => getWorkoutDetail(workoutType, workoutName),
    [workoutType, workoutName]
  );

  const favouriteItem = useMemo(
    () => buildWorkoutFavouriteItem(workoutType, workoutName, workoutDetail?.met),
    [workoutType, workoutName, workoutDetail?.met]
  );

  const dayRecords = useMemo(() => {
    const merged = mergeWorkoutRecords(workoutLogRows, workoutSessionRows);
    return filterDiscoverRecords(merged, workoutName);
  }, [workoutLogRows, workoutSessionRows, workoutName]);
  ...
  useEffect(() => {
    ...
    const qLogs = query(collection(db, "users", uid, "workoutLogs"), where("title", "==", wn), limit(100));
    const qSessions = query(collection(db, "users", uid, "workoutSessions"), where("workout", "==", wn), limit(100));
    ...
    const unsubLogs = onSnapshot(
      ...
      (snap) => {
        ...
        for (const d of snap.docs) {
          const rec = mapDiscoverWorkoutLogDoc(d, wn);
          if (!rec) continue;
          rows.push(rec);
        }
    ...
    const unsubSessions = onSnapshot(
      ...
      (snap) => {
        ...
        const rows: DayRecordRow[] = [];
        for (const d of snap.docs) {
          const rec = mapDiscoverSessionDoc(d, wn);
          if (!rec) continue;
          rows.push(rec);
        }
        rows.sort((a, b) => b.endedAt.getTime() - a.endedAt.getTime());
  ...
  const completeWorkout = async (): Promise<void> => {
    ...
    // Discover sessions do not update plan day progress.
...
export default function FreeWorkoutScreen() {
  const params = useLocalSearchParams<{ type?: string | string[]; name?: string | string[] }>();
  const workoutTypeRaw = useMemo(() => paramOne(params.type), [params.type]);
  const workoutNameRaw = useMemo(() => paramOne(params.name), [params.name]);

  const workoutType = useMemo((): WorkoutType | null => {
    const t = workoutTypeRaw.trim();
    if (t === "Yoga" || t === "Strength" || t === "HIIT" || t === "Cardio") return t;
    return null;
  }, [workoutTypeRaw]);

  const workoutName = useMemo(() => {
    try {
      return decodeURIComponent(workoutNameRaw.trim());
    } catch {
      return workoutNameRaw.trim();
    }
  }, [workoutNameRaw]);

  useEffect(() => {
    if (!workoutType || !workoutName) {
      router.replace("/all-workouts" as any);
      return;
    }
    if (!isCatalogWorkout(workoutType, workoutName)) {
      router.replace("/all-workouts" as any);
    }
  }, [workoutType, workoutName]);
  ...
  return <FreeWorkoutBody workoutType={workoutType} workoutName={workoutName} />;
}
